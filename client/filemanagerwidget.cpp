...
void FileManagerWidget::enableConnectButton()
{
	// 保存当前配置
...
void FileManagerWidget::enableDownloadButton()
{
	// 获取所在行的文件名
...
void FileManagerWidget::processItem(QTableWidgetItem* tableItem)
{
	// 获取所在行第一列name，即文件名
...
void FileManagerWidget::connectOrDisconnect()
{
...
void FileManagerWidget::connectToFtp()
{
	ftp = new QFtp(this);
...
void FileManagerWidget::ftpCommandFinished(int, bool error)
{
#ifndef QT_NO_CURSOR
	setCursor(Qt::ArrowCursor);
#endif
...
void FileManagerWidget::addToList(const QUrlInfo &urlInfo)
{
	QString qSuffix		   = "";
...
void FileManagerWidget::updateDataTransferProgress(qint64 readBytes, qint64 totalBytes)
{
	//progressDialog->setMaximum(totalBytes);
	//progressDialog->setValue(readBytes);
}

void FileManagerWidget::cdToParent()
{
#ifndef QT_NO_CURSOR
	setCursor(Qt::WaitCursor);
#endif
...
void FileManagerWidget::downloadCurrentFile()
{
	// 获取所在行第一列name，即文件名
...
bool FileManagerWidget::downloadFile(QString fileName)
{
...
void FileManagerWidget::cancelDownload()
{
	ftp->abort();			// 中止命令
...
QString FileManagerWidget::getCurrentFileName()
{
	QTableWidgetItem *tempItem = ui.tableWidget_->item(ui.tableWidget_->currentItem()->row(), 0);
...
bool FileManagerWidget::checkDownload(QString fileName)
{
	return downloadFile(fileName);				// 文件未下载先下载
}

void FileManagerWidget::uploadCurrentFile()
{
	// 获取所在行第一列name，即文件名
	QTableWidgetItem *tempItem = ui.tableWidget_->item(ui.tableWidget_->currentItem()->row(), 0);
	QString fileName = tempItem->text();
...
bool FileManagerWidget::uploadFile(QString fileName)
{
